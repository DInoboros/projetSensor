...
import fr.lenours.sensortracker.R;
...
public class ArcProgress extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float finishedSweepAngle = progress <= max ? progress / (float) getMax() * arcAngle : max / (float) getMax() * arcAngle;
        float finishedStartAngle = startAngle;
        paint.setColor(unfinishedStrokeColor);
        canvas.drawArc(rectF, startAngle, arcAngle, false, paint);

        paint.setColor((progress>=max)&&suffixText.equals("pas")?Color.RED:finishedStrokeColor);
        canvas.drawArc(rectF, finishedStartAngle, finishedSweepAngle, false, paint);

        String text = String.valueOf(getProgress());
        if (!TextUtils.isEmpty(text)) {
            textPaint.setColor(textColor);
            textPaint.setTextSize(textSize + 30);
            ...
            //canvas.drawText(Integer.parseInt(text)<1000?text+"m": Extra.decimalFormat(Double.parseDouble(text)/1000,1)+"Km", (getWidth() - textPaint.measureText(text)) / 2.0f, textBaseline, textPaint);
            textPaint.setTextSize(bottomTextSize - 10);
           // canvas.drawText(!(max==1)?max+"":"",(getWidth() - textPaint.measureText(max+"")) / 2.0f, textBaseline + 30,textPaint);
